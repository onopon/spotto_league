...
class PonnoNotify:
    ...
    def __init__(self, access_token: Optional[str] = None) -> None:
        self._access_token = (
            access_token
            if access_token
            else settings.LINE_NOTIFY_ACCESS_TOKEN_HASH[settings.ENV]
        )
    ...
    def notify_about_cancel_league(self, league_id: int) -> None:
        text = LeagueTemplateSendMessage.get_for_push_about_cacnel_league(
            league_id=league_id
        ).text
        self.execute(text)
    ...
    def notify_about_join_end_at_deadline(self) -> None:
        ...
        messages.pop(-1)  # 最後の \n を除く
        first_messages = ["締め切りの近い練習会があるみたいだよ！", "参加表明がまだの方はお早めにね！\n"]
    ...
    def notify_about_recruiting_league_information(self, league_ids: List[int]) -> None:
        ...
        if not messages:
            return
        first_messages = ["募集中の練習会が追加されたみたいだよ！", "ご都合の合う方はどしどし参加表明ボタンを押してね！\n"]
        self.execute("\n".join(first_messages + messages))
